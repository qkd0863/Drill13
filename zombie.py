...
class Zombie:
    ...
    def set_escape_location(self):
        self.tx, self.ty = play_mode.boy.x + 500, play_mode.boy.y + 500
        return BehaviorTree.SUCCESS
    ...
    def build_behavior_tree(self):
        ...
        a5 = Action('도망간다', self.set_escape_location)
        a6 = Action('살짝 도망간다', self.move_slightly_to, play_mode.boy.x + 500, play_mode.boy.y + 500)
        ...
        c2 = Condition('공의 개수가 더 많은가?', self.compare_ball_win)
        c3 = Condition('공의 개수가 더 적은가?', self.compare_ball_lose)
        a4 = Action('소년으로 이동', self.move_to_boy)

        SEQ_chase_boy = Sequence('공의 개수가 더 많으니 소년을 추적한다', c1, c2, a4)
        SEQ_run = Sequence('도망', c1, c3, a6)

        SEL_chase_or_run = Selector('추적 또는 도망', SEQ_chase_boy, SEQ_run)

        root = SEL_chase_or_wander = Selector('추적 또는 배회', SEL_chase_or_run, SEQ_wander)
